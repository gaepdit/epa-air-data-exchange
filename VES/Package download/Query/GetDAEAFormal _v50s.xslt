    ...
    <xsl:template match="v1:DataSet">
		...
		<AirDAFormalEnforcementActionData xmlns="http://www.exchangenetwork.net/schema/icis/5">
			...
			<AirDAFormalEnforcementAction>
					...
					<xsl:for-each select="//v1:record[v1:TransactionID=$TransactionID][v1:EnforcementActionId=$EnforcementActionId][not(self::v1:record/v1:AirFacilityId=following-sibling::v1:record/v1:AirFacilityId)]">
					...
					<xsl:for-each select="//v1:record[v1:TransactionID=$TransactionID][v1:EnforcementActionId=$EnforcementActionId][not(self::v1:record/v1:AirDAFinalOrderId=following-sibling::v1:record/v1:AirDAFinalOrderId)]">
					<AirDAFinalOrder>
						<FinalOrderIdentifier>
							<xsl:value-of select="v1:AirDAFinalOrderId" />
						</FinalOrderIdentifier>
						<FinalOrderTypeCode>
					...
					<xsl:for-each select="//v1:record[v1:TransactionID=$TransactionID][v1:EnforcementActionId=$EnforcementActionId][not(self::v1:record/v1:ContactId=following-sibling::v1:record/v1:ContactId)]">
